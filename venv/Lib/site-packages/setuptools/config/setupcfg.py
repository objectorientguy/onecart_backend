...
def _warn_accidental_env_marker_misconfig(label: str, orig_value: str, parsed: list):
    ...
    but because there is no newline, it's parsed as two requirements.txt
    with a semicolon as separator.

    Therefore, if:
        * input string does not contain a newline AND
        * parsed result contains two requirements.txt AND
    ...
    with contextlib.suppress(InvalidRequirement):
        ...
        if req.marker is not None:
            msg = (
                f"One of the parsed requirements.txt in `{label}` "
...
class ConfigOptionsHandler(ConfigHandler["Distribution"]):
    ...
    def _parse_requirements_list(self, label: str, value: str):
        # Parse a requirements.txt list, either by reading in a `file:`, or a list.
